...
public class GoalControllerTest {
    ...
    public void create() throws Exception {
        CreateGoalDto goalDto = new CreateGoalDto("Test title", 99f, "<=", false, new ArrayList<>(Math.toIntExact(latestStat().getId())));
        assertThat(controller.create(goalDto)).isEqualTo("Done!");
    }

    @Test
    public void read() throws Exception{
        assertThat(controller.listAll().stream().map(Goal::getId).filter(Long -> Long.equals(latestGoal().getId())).findFirst().isPresent());
    }

    @Test
    @Transactional
    public void delete() throws Exception{
        Long idActedOn = latestGoal().getId();
